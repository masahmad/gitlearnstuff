...
public $returnurl="http://vandenboschict.nl/classes/facebooklogindemo.php";
private $user_token;
...
public  $profile; 
public $username;
protected $twitterclass;
public function __construct($config){
$this->config=$config;
$this->twitterclass=new Twitter($config['client_id'], $config['client_secret']);
}
public function login($redirurl=""){
if (!empty($redirurl)) $this->returnurl=$redirurl;
$token=$this->twitterclass->oAuthRequestToken($this->returnurl);
$this->twitterclass->oAuthAuthorize($token['oauth_token']);
}

public function gettoken($token, $verifier){
$token = $this->twitterclass->oAuthAccessToken($token, $verifier);
      
      if (empty($token) || sizeof($token) < 4)
      {
        // Shouldn't happen, but I've experienced it before
        throw new Exception('Invalid response when getting access_token.');
      }else{
		$this->user_token=$token['oauth_token'];
		$this->user_token_secret=$token['oauth_token_secret'];
	  }
	  return $token;
}
public function getprofile(){
$this->profile=$this->twitterclass->accountVerifyCredentials();
$this->id=$this->profile['id'];
$this->username=$this->profile['screen_name'];
$this->fullname=$this->profile['name'];

$this->profile=array(
				'id'=>$this->profile['id'],
				'first_name'=>strtok($this->profile['name']," "),
				
				'last_name'=>substr($this->profile['name'], strpos($this->profile['name'], " ") + 1),
				'full_name'=>$this->profile['name'],
				'nickname'=>$this->profile['screen_name'],
				'name' => $this->profile['name'],
				'profilepic'=>$this->profile['profile_image_url_https'],
				'village'=>$this->profile['location'],
				'tagline'=>$this->profile['description'],
				'website'=>$this->profile['url']
			); 
return $this->profile;
}
